...
max_time = 120*60
start_time = time.time()                                                                      # JEDZIEMY Z TYM KIOSKIEM!

"""1 - przypisanie identyfikatorow wezlow do segmentow sieci rzecznej - Warstwa rzek musi posiadac atrybuty [START_ID 
i END_ID] oznaczajace identyfikatory wezlow """
...
with arcpy.da.UpdateCursor(rzeki, ['OBJECTID', 'START_ID', 'KOD', 'ID_HYD_R_10', 'PRZEBIEG'])as cur:
    for row in cur:
        war = (row[1], row[3])
...
while 0 in iteracja and (time.time() - start_time) < max_time:
    lista2 = []
    with arcpy.da.SearchCursor(rzeki, ['END_ID', 'KOD', 'ID_HYD_R_10', 'TYP_O'])as cur:
        for row in cur:
            lista2.append((row[0], row[1], row[2], row[3]))
    del cur

    with arcpy.da.UpdateCursor(rzeki, ['OBJECTID', 'START_ID', 'KOD', 'ID_HYD_R_10', 'TYP_O'])as cur:
        for row in cur:
            if row[2] == 0:
                kody = []
                rzad = []
                ...
                for tupla in lista2:
                    if row[1] == tupla[0]:
                        kody.append((tupla[1], tupla[2], tupla[3]))
                        rzad.append(tupla[1])
                        i += 1
                ...
                if row[4] >= 4:
                    ...
                    elif 0 not in rzad and rzad[0] == rzad[1] and max(syfony) < 4:
                        row[2] = rzad[0]+1
                        cur.updateRow(row)
                    elif 0 not in rzad and rzad[0] != rzad[1] and max(syfony) < 4:
                        row[2] = max(rzad)
                    ...
                    if 0 not in rzad and rzad_zr == max(styk):
                        row[2] = rzad_zr+1
                        cur.updateRow(row)
                    elif 0 not in rzad and rzad_zr > max(styk):
                        row[2] = rzad_zr
                        cur.updateRow(row)
                    elif 0 not in rzad and rzad_zr < max(styk):
                        row[2] = rzad_zr
    ...
    iteracja = []
    with arcpy.da.SearchCursor(rzeki, ['KOD'])as cur:
        for row in cur:
